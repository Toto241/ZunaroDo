...
- **Hover-Tooltips** — `core/tooltip.py` (`Tooltip`/`attach_tooltip`, stdlib-only, defensiv). Additiv eingesetzt (statische Labels bleiben): Hilfetext-Hover an den Settings-Eingabefeldern (`gui.py`) sowie an Aktions-/Link- und den gekuerzten Referenz-Buttons des Control Panels (`tools/control_panel.py`). Schliesst die letzte Audit-Saeule (#5) aus `docs/KONFIG_AUDIT.md`.
